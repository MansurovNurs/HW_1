...
class Magic(Hero):
    def __init__(self, name, health, damage, booster):
    ...
    def booster(self, value):
        self.__booster = value

    def apply_super_power(self, boss: Boss, heroes: list):
        for hero in heroes:
            if round_number < 5:
                hero.damage += self.booster
                print(f'mag {self.name} boosted: {hero.name}')
    ...
    def apply_super_power(self, boss: Boss, heroes: list):
        for hero in heroes:
            if hero.health == 0:
                ...
                break
        print(f'Witcher {self.name} sacrificed: {self.health}')
...
class Hacker(Hero):
    ...
    def apply_super_power(self, boss: Boss, heroes: list):
        if round_number % 2 == 0:
            ...
            print(f'Hacker {self.name} stole {self.__steal_amount} from Boss and gave it to {target_hero.name}')
...
def start_game():
    ...
    witcher = Witcher('Smertnik', 350, 0)



    heroes_list = [warrior_1, doc, witcher, warrior_2, magic, berserk, assistant]
